...
def test_spending_by_category_correct_answer(user_transactions):
    expected = pd.DataFrame(
        {"Дата платежа": [pd.to_datetime("01.06.2023", format="%d.%m.%Y")],
         "Категория": ["Мебель"]})
...
def test_spending_by_category_incorrect_args(user_transactions):
    assert spending_by_category("incorrect", "Мебель", "2023-06-22 00:00:00") == None
    assert spending_by_category(user_transactions, 123, "2023-06-22 00:00:00") == None
    assert spending_by_category(user_transactions, "Мебель", "2023-06-22") == None